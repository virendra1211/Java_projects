...
The token includes their username and expiration time, signed with a secret key. This token is returned to
 the client,
 who stores it locally. For all subsequent requests, the client sends the token in the Authorization header 
 as a Bearer token.
...
***** Refresh Token ******
*A refresh token is a long-lived token that allows a user to obtain a new access token after the 
*current one expires. 
*When a user logs in, we issue both an access token (short-lived) and a refresh token (long-lived).
The access token is used for calling protected APIs. When it expires, the client sends 
the refresh token to a special endpoint like /auth/refresh.
